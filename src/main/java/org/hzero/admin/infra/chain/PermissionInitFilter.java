...
import io.choerodon.admin.infra.chain.SagaInitFilter;
...
public class PermissionInitFilter implements InitFilter, Ordered {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PermissionInitFilter.class);

    @Override
    public void doFilter(InitChain chain, InitChainContext context) {
        Service service = context.getService();
        LOGGER.info(">>>>>>>>>>>>>>>> start refresh permisison. service:{} <<<<<<<<<<<<<<<<<<<<<<", service);
