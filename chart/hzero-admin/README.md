...
```bash
$ helm install c7n/hzero-admin --name hzero-admin
```
...
`image.repository` | 镜像库地址 | `registry.choerodon.com.cn/choerodon/hzero-admin`
...
$ helm install c7n/hzero-admin \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=hzero-admin  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
    --name hzero-admin
```
