...
`image.repository` | 镜像库地址 | `registry.choerodon.com.cn/choerodon/base-service`
...
`deployment.managementPort` | 服务管理端口 | `8061`
...
`service.name` | service名称 | `base-service`
...
$ helm install c7n/base-service \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=base-service  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
    --name base-service
```

## 验证部署
```bash
curl -s $(kubectl get po -n c7n-system -l choerodon.io/release=base-service -o jsonpath="{.items[0].status.podIP}"):8061/actuator/health | jq -r .status
```
出现以下类似信息即为成功部署
